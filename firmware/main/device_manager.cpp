...
struct device_entry_t {
    ...
    uint32_t primary_device_type = 0;
};
...
static void load_from_disk(void)
{
    ...
    cJSON_ArrayForEach(dev_json, devices) {
        ...
        cJSON *pdt = cJSON_GetObjectItemCaseSensitive(dev_json, "deviceType");
        if (cJSON_IsNumber(pdt)) dev.primary_device_type = (uint32_t)pdt->valuedouble;
...
esp_err_t device_manager_set_primary_device_type(uint64_t node_id, uint32_t device_type_id)
{
    xSemaphoreTake(s_mutex, portMAX_DELAY);
    auto *dev = find_device(node_id);
    if (dev) dev->primary_device_type = device_type_id;
    xSemaphoreGive(s_mutex);
    return dev ? ESP_OK : ESP_ERR_NOT_FOUND;
}
...
char *device_manager_get_all_json(void)
{
    ...
    for (const auto &dev : s_devices) {
        ...
        cJSON_AddNumberToObject(dobj, "deviceType",  (double)dev.primary_device_type);
...
static constexpr uint32_t kDevTypeOnOffLight      = 0x0100;
static constexpr uint32_t kDevTypeDimmableLight   = 0x0101;
