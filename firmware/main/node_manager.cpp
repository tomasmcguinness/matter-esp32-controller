...
static void load_from_disk(void)
{
    ...
    cJSON_Delete(root);
    ESP_LOGI(TAG, "Loaded %u node(s), %u edge(s)", (unsigned)s_nodes.size(), (unsigned)s_edges.size());
}
...
esp_err_t node_manager_delete(const char *node_id)
{
    ...
    for (auto it = s_nodes.begin(); it != s_nodes.end(); ++it) {
        if (it->id == node_id) { s_nodes.erase(it); break; }
    }
    xSemaphoreGive(s_mutex);
    return node_manager_persist();
}
...
esp_err_t node_manager_persist(void)
{
    ...
    free(text);
    if (result == ESP_OK)
        ESP_LOGI(TAG, "Persisted %u node(s)", (unsigned)s_nodes.size());
    else
        ESP_LOGE(TAG, "Failed to persist nodes");
    return result;
}
